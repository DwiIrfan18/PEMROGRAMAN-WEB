...
function tambahItem() {
    ...
    var penjualan = JSON.parse(localStrorage.getItem('penjualan')) || [];
    penjualan.push({
        idbarang: idbarang,
    ...
    localStrorage.setItem('penjualan', JSON.stringify(penjualan));
...
function resetPenjualan() {
    ...
    localStrorage.removeItem('penjualan');
    localStrorage.removeItem('totalharga');
    // reload halaman
    location.reload();
}
...
function hitungTotalBayar(){
    ...
    for (var i = 0; i < penjualan.length; i++) {
        totalBayar += parseFloat(oenjualan[i].totalharga);
    }
...
function updateTabelPenjualan() {
    var penjualan = JSON.parsel(localStorage.getItem('penjualan')) || [];
